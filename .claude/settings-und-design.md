...
| Datum | Was |
|---|---|
| 25.08.2026 | **Lizenz final entschieden: MIT.** `LICENSE`-Datei angelegt, README-Badge/Abschnitt angeglichen, `entscheidungen-richard.md` Punkt 1 abgehakt. Begründung: Journal-App ist technisch leicht nachbaubar, „lokal + kostenlos" passt ohnehin schlecht zu klassischer Enterprise-Monetarisierung — ein späteres Agent/Skill/Workflow-Produkt wäre der eigentliche kommerzielle Hebel, unabhängig von diesem Repo. PromptPeter-Branding ins README eingebaut (Maskottchen-Bild, kurze Vorstellung, Schluss-Zeile), GitHub-Repo-Beschreibung/Themen gesetzt |
| 25.08.2026 | **Logo/Maskottchen final entschieden: PromptPeter**, `brand/promptpeter.svg`. Loser Kugelschreiber-Skizzenstil (Referenz: „Wilfredo Batulan"-artige Zeichnungen — große runde Brille, starrer/leicht besorgter Blick, wacklige, zweifach nachgezogene Linie statt sauberer Vektorkurve, simuliert über SVG-`feTurbulence`/`feDisplacementMap`). Einziger Marken-Bezug: eine Faden-Schlaufe statt Krawatte am Kragen, knüpft an die zuvor entworfene „Faden"-Wortmarke an. Nutzername/Community-Figur ebenfalls „PromptPeter" (statt „PromptEngineer" pur — kollidiert stark auf GitHub; „OKF" vermieden — kollidiert mit Googles eigenem Open Knowledge Format). Noch offen: Einbindung in README/App-Header, falls gewünscht |
| 25.08.2026 | `README.md` und `CONTRIBUTING.md` fürs Repo geschrieben — Vorbereitung auf GitHub-Veröffentlichung (Recherche zu guten OSS-READMEs eingeflossen: Screenshot-Platzhalter, 30-Sekunden-Regel, Roadmap mit „Agent/Skill/Workflow/Prompt-Bibliothek" als nächstem großem Punkt). Beide Manifeste (`manifest.en.md`/`manifest.de.md`) nachgezogen: Extension ist jetzt „erledigt" statt „geplant", Exportformat-Zahl korrigiert (9→11). Lizenz bewusst als „TBD" markiert, keine Entscheidung vorweggenommen |
| 25.08.2026 | Versionsnummer eingeführt (`APP_VERSION`, dezent im Header neben dem Titel) — für spätere GitHub-Releases/README. Start bei `1.0.0`, bei Änderungen hier hochzählen |
